# File Attachments (F-22) — Audit Report (Cycle 2)
Date: 2026-04-09
Tasks covered: T-108, T-109, T-110, T-111, T-112, T-113, T-114, T-115, T-116

## Cycle 1 Fix Verification

Both medium-severity issues from Cycle 1 have been fixed:

1. **Bug #1 FIXED:** `size` field now included in context builder attachment mapping — `src/lib/contextBuilder.ts:44` includes `size: att.size`.
2. **Bug #1 (cont.) FIXED:** `size` field now included in chat route attachment mapping — `src/app/api/llm/chat/route.ts:213` includes `size: a.size`.
...
- **PASS:** Token estimation accounts for attachment size — `estimateMessageTokens` reads `att.size` at line 8, and the `size` field is now correctly mapped.
...
- **PASS:** `__tests__/lib/providers/attachmentFormatter.test.ts` exists — covers all provider formats, text decoding, and markdown/CSV handling.
- **PASS:** `__tests__/api/llm-chat.test.ts` has attachment validation tests — valid save, >5 files, >5MB, >10MB total, invalid MIME.
- **PASS:** `__tests__/api/import-export.test.ts` has attachment round-trip tests (export includes, import restores).

## Bug Detection

### Bug #1 (Cycle 1) — RESOLVED
The `size` field is now correctly included in both `contextBuilder.ts` and `chat/route.ts` attachment mappings. Token estimation works correctly.

### Bug #2 (Cycle 1, Low) — Still Present
**File:** `src/components/chat/FileUploadArea.tsx:133`
**Description:** `URL.createObjectURL(file)` is called in the render for image thumbnails but `URL.revokeObjectURL` is never called. Blob URLs accumulate in memory during a session. Low severity — typical usage with ≤5 files and the fact that files are cleared on send means negligible practical impact.
**Severity:** Low

### Bug #3: Test fixtures missing `size` field (Low)
**File:** `__tests__/lib/providers/attachmentFormatter.test.ts:5-37`
**Description:** All `LLMAttachment` test fixtures (`imageAttachment`, `pdfAttachment`, `textAttachment`, etc.) are typed as `LLMAttachment` but omit the required `size` field. This produces TypeScript type errors (`TS2741: Property 'size' is missing`). The tests still pass at runtime since `formatAttachmentsForProvider` doesn't use the `size` field, but the fixtures don't match the interface contract.
**Severity:** Low — tests pass, `npm run build` succeeds (test files excluded from build), functionality is correct.
...
- **LLMAttachment type:** Defined in `src/lib/providers/types.ts` with `size` field — matches Architecture Delta §3.4 (note: delta spec doesn't include `size` in `LLMAttachment` but the implementation includes it for token estimation, which is an acceptable enhancement).
